...
class Node:
    ...
    def run_command(self):
        ...
        image = "/tmp/gluon-ffda-2.3~20200811-x86-64.img"
        ...
        for network in self.networks:
            ...
            start_command.extend(
                [
                    f"-device {nic_driver},addr=0x{10 + network.id},netdev={network.name}",
    ...
    def run_hooks(self):
        if self.configmode:
            ...
            self.execute("uci set gluon-setup-mode.@setup_mode[0].configured='1'")
            self.execute("uci set gluon-setup-mode.@setup_mode[0].enabled='0'")
            self.execute("uci changes")
            self.execute("uci commit")
            self.execute("sync")
            self.execute("sleep 10")
    ...
    def succeed(self, *commands: str) -> str:
        ...
        for command in commands:
            self.log(f"Executing {command}", bold=True)
...
if __name__ == "__main__":
    ...
    a.succeed("cat /lib/libfstools.so | grep \"rootfs_data\"")

    #a.succeed("test -d /var/gluon/setup-mode")
    a.succeed("mount")
    ...
    a.succeed("wget -4 https://darmstadt.freifunk.net")

    #a.succeed("opkg update")
